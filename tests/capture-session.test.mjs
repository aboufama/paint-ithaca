...
test('one click completes the photo bloom without a release event', () => {
  ...
  session.advance(1400); assert.equal(session.progress,.5); assert.equal(session.state,'painting');
  session.advance(1400); assert.equal(session.progress,1); assert.equal(session.state,'review'); assert.equal(session.active,false);
});
