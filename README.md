...
- **Release** stops new paint. Wet edges settle for half a second and the same view becomes the finished moment. Clips are capped at 12 seconds.
...
`watercolor.js` evolves mobile pigment, wetness, and settled pigment in WebGL2. `brush.js` paints an irregular bristle mask over time. Only the masked areas receive color; the surrounding pale image remains useful for framing. The saved video records this composited painting canvas.

This is an independent implementation inspired by [Sudo Aquarelle](https://sudoaquarelle.com/), not a redistribution of its source. It uses a simpler pigment model with a Kubelka–Munk reflectance approximation.
